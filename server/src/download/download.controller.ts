...
export class DownloadController {
    ...
    async getExcelDownload(@Res() response: Response,
        ...
        switch (custom_flag) {
            case 'rmiOilIndex':
                workbook = await this.downloadService.rmiExcel(category_ID, custom_flag, paginationDto)
                console.log('in controller')
                return workbook.xlsx.write(response)
            case 'custom':
                // call custom service
                workbook = await this.downloadService.customExcel(category_ID, custom_flag, paginationDto)
                return workbook.xlsx.write(response)

            case 'AEO2021':
                // call AEO2021 service
                workbook = await this.downloadService.aeo2021Excel(category_ID, custom_flag, paginationDto)
                return workbook.xlsx.write(response)
            case 'kaya':
                // call kaya service
                workbook = await this.downloadService.kayaExcel(category_ID, custom_flag, paginationDto)
                return workbook.xlsx.write(response)
            case 'EIA':
                // call EIA service
                workbook = await this.downloadService.eiaExcel(category_ID, custom_flag, paginationDto)
                return workbook.xlsx.write(response)
